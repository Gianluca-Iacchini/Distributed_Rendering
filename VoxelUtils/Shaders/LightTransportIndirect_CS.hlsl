...
StructuredBuffer<uint4> gLitClusters : register(t1, space4);

ByteAddressBuffer gVisibleFaceCounter : register(t0, space5);
StructuredBuffer<uint> gIndirectLightVisibleFacesIndices : register(t1, space5);
...
void CS( uint3 DTid : SV_DispatchThreadID, uint3 threadGroupId : SV_GroupThreadID)
{
    ...
    if (threadGroupId.y == 0)
    {
        ...
        gRadianceForVisibleFaceIdx[threadID] = uint2(idx, packedRadx);
    }
}